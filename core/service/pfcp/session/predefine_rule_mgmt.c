...
struct pf_rule_table_head pf_rule_head;
struct white_list_table_head wl_head;

struct predefine_table *pf_rule_table_get(uint32_t index)
{
	if(index >= MAX_PF_RULE_TABLE)
		return NULL;
	else
		return &pf_rule_head.pf_table[index];
}

int64_t pf_rule_table_init(uint32_t pf_rule_num)
{
	uint32_t index = 0;
	int pool_id = -1;
	...
	if (0 == pf_rule_num) {
		LOG(SESSION, ERR,
			"Abnormal parameter, pf_rule_num: %u.", pf_rule_num);
		return -1;
	}

	max_num = pf_rule_num;
	LOG(SESSION, RUNNING,
			"init pf_rule, sizeof(pf_rule): %lu  max_num: %u.",
			sizeof(struct predefine_table), max_num);
	size = sizeof(struct predefine_table) * max_num;
	pf_rule_tbl = ros_malloc(size);
	if (NULL == pf_rule_tbl) {
		LOG(SESSION, ERR,
			"init pf_rule failed, no enough memory, max number: %u =",max_num);
		return -1;
	}
	ros_memset(pf_rule_tbl, 0, sizeof(struct predefine_table) * max_num);

	for (index = 0; index < max_num; ++index) {
		pf_rule_tbl[index].index = index;
		ros_rwlock_init(&pf_rule_tbl[index].lock);
	}
	...
	pf_rule_head.pool_id = pool_id;
	pf_rule_head.pf_table = pf_rule_tbl;
	pf_rule_head.max_num = max_num;
	ros_rwlock_init(&pf_rule_head.lock);
	ros_atomic32_set(&pf_rule_head.use_num, 0);
...
uint32_t pf_rule_table_create(void)
{
	struct predefine_table *pf_rule_tbl = NULL;
	uint32_t				i;


	for(i=0;i<MAX_PF_RULE_TABLE;i++)
	{
		pf_rule_tbl = &pf_rule_head.pf_table[i];
		if(pf_rule_tbl->inuse == 0)
			break;
	}
	if(i>=MAX_PF_RULE_TABLE)
	{
		LOG(SESSION, ERR,
			"no pf_rule table available[%d]",i);
		return -1;
	}
...
struct predefine_table *pf_rule_table_search(uint8_t *rule_name)
{
	struct predefine_table *pf_rule_tbl = NULL;
	uint32_t				i;
	...
	{
		pf_rule_tbl = &pf_rule_head.pf_table[i];
		if(pf_rule_tbl->inuse)
		{
			if(!strcmp((char *)(pf_rule_tbl->predefine_name),(char *)rule_name))
				break;
		}
	}
	if(i>=MAX_PF_RULE_TABLE)
...
int pf_rule_table_show(struct cli_def *cli, int argc, char **argv)
{
	struct predefine_table *pf_rule_tbl = NULL;
	char 					buf[256];
	uint32_t				i,j,k;
	...
	for(i=0;i<MAX_PF_RULE_TABLE;i++)
	{
		...
		if(pf_rule_tbl->inuse)
		{
			cli_print(cli, "index:%d predefine_name:%s activate:%d\r\n",pf_rule_tbl->index,pf_rule_tbl->predefine_name,pf_rule_tbl->activate);
			if(pf_rule_tbl->pdr_num > 0)
			{
				k=0;
				for(j=0;j < pf_rule_tbl->pdr_num && j < MAX_PF_RULE_PDR_TABLE;j++)
				{
					k+=sprintf(buf+k,"[index:%d id:%d] ",pf_rule_tbl->pdr_index[j],pf_rule_tbl->pdr_id[j]);
				}
				cli_print(cli, "pdr_num:%d %s\r\n",pf_rule_tbl->pdr_num,buf);
			}
			else
			{
				cli_print(cli, "pdr_num:%d\r\n",pf_rule_tbl->pdr_num);
			}

			cli_print(cli, "bar_num:%d bar_index:%d bar_id:%x\r\n",pf_rule_tbl->bar_num,pf_rule_tbl->bar_index,pf_rule_tbl->bar_id);

			if(pf_rule_tbl->far_num > 0)
			{
				k=0;
				for(j=0;j < pf_rule_tbl->far_num && j < MAX_PF_RULE_FAR_TABLE;j++)
				{
					k+=sprintf(buf+k,"[index:%d id:%x] ",pf_rule_tbl->far_index[j],pf_rule_tbl->far_id[j]);
				}
				cli_print(cli, "far_num:%d %s\r\n",pf_rule_tbl->far_num,buf);
			}
			else
			{
				cli_print(cli, "far_num:%d\r\n",pf_rule_tbl->far_num);
			}

			if(pf_rule_tbl->qer_num > 0)
			{
				k=0;
				for(j=0;j < pf_rule_tbl->qer_num && j < MAX_PF_RULE_QER_TABLE;j++)
				{
					k+=sprintf(buf+k,"[index:%d id:%x] ",pf_rule_tbl->qer_index[j],pf_rule_tbl->qer_id[j]);
				}
				cli_print(cli, "qer_num:%d %s\r\n",pf_rule_tbl->qer_num,buf);
			}
			else
			{
				cli_print(cli, "qer_num:%d\r\n",pf_rule_tbl->qer_num);
			}

			if(pf_rule_tbl->urr_num > 0)
			{
				k=0;
				for(j=0;j < pf_rule_tbl->urr_num && j < MAX_PF_RULE_QER_TABLE;j++)
				{
					k+=sprintf(buf+k,"[index:%d id:%x] ",pf_rule_tbl->urr_index[j],pf_rule_tbl->urr_id[j]);
					urr_table_show(urr_get_table(pf_rule_tbl->urr_index[j]));
				}
				cli_print(cli, "urr_num:%d %s\r\n",pf_rule_tbl->urr_num,buf);
			}
			else
			{
				cli_print(cli, "urr_num:%d\r\n",pf_rule_tbl->urr_num);
			}

			if(pf_rule_tbl->quote_pdr_num > 0)
			{
				k=0;
				for(j=0;j < pf_rule_tbl->quote_pdr_num && j < MAX_PF_RULE_QUOTE_PDR_TABLE;j++)
				{
					k+=sprintf(buf+k,"[index:%d] ",pf_rule_tbl->quote_pdr_index[j]);
				}
				cli_print(cli, "quote_pdr_num:%d %s\r\n",pf_rule_tbl->quote_pdr_num,buf);
			}
			else
			{
				cli_print(cli, "quote_pdr_num:%d\r\n",pf_rule_tbl->quote_pdr_num);
			}
			cli_print(cli, "\r\n");
		}
	}
...
uint32_t pf_rule_table_delete(uint32_t index)
{
	struct predefine_table *pf_rule_tbl = NULL;
	...
	{
		LOG(SESSION, ERR,
			"pf_rule table index is invaild[%d]",index);
		return -1;
	}
	pf_rule_tbl = &pf_rule_head.pf_table[index];
	memset(pf_rule_tbl,0,sizeof(struct predefine_table));
	pf_rule_tbl->index = index;

	return 0;
}

uint32_t pf_rule_table_clear(void)
{
	struct predefine_table *pf_rule_tbl = NULL;
	uint32_t				index;

	memset(pf_rule_head.pf_table,0,sizeof(struct predefine_table)*MAX_PF_RULE_TABLE);

	for (index = 0; index < MAX_PF_RULE_TABLE; ++index) {
		pf_rule_tbl=&pf_rule_head.pf_table[index];
		pf_rule_tbl->index=index;
		ros_rwlock_init(&pf_rule_tbl[index].lock);
	}
